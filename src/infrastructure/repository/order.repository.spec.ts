...
import OrderItemModel from "../db/sequelize/model/order-item/order-item.model";
...
describe("Order repository test", () => {
  ...
  it("should find all orders", async () => {
    ...
    const product2 = new Product("123456", "Product 1", 10);
    ...
    const order2 = new Order("123456", customer2.id, [orderItem2]);

    await orderRepository.create(order2);

    const orderFounded = await orderRepository.findAll()
    
    expect(orderFounded).toEqual([order, order2])
  })
});
